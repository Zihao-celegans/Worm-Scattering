...
% compute contrast for individual worm
contrastData = calculateContrastStats(worm_bg_data);
...
contrastData = contrastData(strcmp(contrastData.Category, 'contrast'), :);
...
if (is_forward == true)
    ...
    contrastData.view_LED_Theta = round(comput_incident_angle(contrastData.Position, radius_LED, max_pos_read_cm, mini_dist_mm),1);
    contrastData.Theta = contrastData.view_LED_Theta;
else
    wormData.view_LED_Theta = round(comput_incident_angle(-wormData.Position, radius_LED, max_pos_read_cm, mini_dist_mm),1);
    ...
    contrastData.view_LED_Theta = round(comput_incident_angle(-contrastData.Position, radius_LED, max_pos_read_cm, mini_dist_mm),1);
    contrastData.Theta = 180 - contrastData.view_LED_Theta;
...
contrastData = innerjoin(contrastData, interp_irradiance_data, 'Keys', 'view_LED_Theta');
...
if (~isequal(wormData.Theta, backgroundData.Theta) || ~isequal(wormData.Theta, contrastData.Theta))
    error('Theta values for worm, background, and contrast do not match.');
end

worm_ScatFunc.mean = wormData.ScatFunc_Mean;
worm_ScatFunc.std = wormData.ScatFunc_Std;
worm_ScatFunc.count = wormData.GroupCount;

bg_ScatFunc.mean = backgroundData.ScatFunc_Mean;
bg_ScatFunc.std = backgroundData.ScatFunc_Std;
bg_ScatFunc.count = backgroundData.GroupCount;

contrast_out.mean = contrastData.mean_Contrast;
contrast_out.std = contrastData.std_Contrast;
contrast_out.count = contrastData.GroupCount;
