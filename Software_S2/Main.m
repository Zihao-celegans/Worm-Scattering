...
%% Calculate pixel irradiance coefficient (PIC) unit: ADU*mm^2*sec^-1*µW^-1
% Area of power meter sensor OP-2 VIS, unit: mm^2
D_PM_sensor = 7.9; % unit: mm
Area_PM_sensor = pi*(D_PM_sensor/2)^2;

% PIC for red
% OFF
f_OFF_red = append(f_dataset_S1, '\PIC\red\off');

% ON
f_ON_red =  append(f_dataset_S1, '\PIC\red\on');

% Exposure time (sec)
expo_red = 1/120;

% optical power measured when LED is OFF
opt_pow_OFF_red = 0.10;

% optical power measured when LED is ON
opt_pow_ON_red = 3.43;

% compute pixel irradiance coefficient (PIC) unit: ADU*mm^2*sec^-1*µW^-1
PIC_red = computePICfromFile(f_OFF_red, f_ON_red, expo_red, opt_pow_OFF_red, opt_pow_ON_red, Area_PM_sensor);

% PIC for green
% OFF
f_OFF_green = append(f_dataset_S1, '\PIC\green\off');

% ON
f_ON_green = append(f_dataset_S1, '\PIC\green\on');

% Exposure time (sec)
expo_green = 1/120;

% optical power measured when LED is OFF
opt_pow_OFF_green = 0.14;

% optical power measured when LED is ON
opt_pow_ON_green = 2.83;

% compute pixel irradiance coefficient (PIC) unit: ADU*mm^2*sec^-1*µW^-1
PIC_green = computePICfromFile(f_OFF_green, f_ON_green, expo_green, opt_pow_OFF_green, opt_pow_ON_green, Area_PM_sensor);

% PIC for blue
% OFF
f_OFF_blue = append(f_dataset_S1, '\PIC\blue\off');

% ON
f_ON_blue = append(f_dataset_S1, '\PIC\blue\on');

% Exposure time (sec)
expo_blue = 1/250;

% optical power measured when LED is OFF
opt_pow_OFF_blue = 0.19;

% optical power measured when LED is ON
opt_pow_ON_blue = 6.795;

% compute pixel irradiance coefficient (PIC) unit: ADU*mm^2*sec^-1*µW^-1
PIC_blue = computePICfromFile(f_OFF_blue, f_ON_blue, expo_blue, opt_pow_OFF_blue, opt_pow_ON_blue, Area_PM_sensor);
