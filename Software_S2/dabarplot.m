...
for g = 1:num_groups 
    ...
    means   = mean(Y(Gi==g,:),1, 'omitnan');   
    
    if ischar(confs.errorbars) && strcmp(confs.bartype,'grouped')
        if confs.errorbars == "WSE"
            Yad = Y(Gi==g,:) - repmat(mean(Y(Gi==g,:), 2, 'omitnan'), [1, size(Y(Gi==g,:),2)]);
            er  = std(Yad, 'omitnan')/sqrt(size(Y(Gi==g,:),1));
        elseif confs.errorbars == "SE"
            er = std(Y(Gi==g,:),[],1,'omitnan')/sqrt(size(Y(Gi==g,:),1));
            % disp("er = ")
            % disp(er)
        elseif confs.errorbars == "SD"
            er = std(Y(Gi==g,:),[],1, 'omitnan');
        end    
    end
